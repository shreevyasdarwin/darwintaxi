...
class API_Controller extends CI_Controller
{
    ...
    public function _allow_methods(array $methods)
    {
        ...
        if (in_array(strtolower($REQUEST_METHOD), $this->allowed_http_methods))
        {
            ...
            } else
            {
                ...
                $this->_response(['status' => FALSE, 'error' => 'Unknown method'], $this->HTTP_METHOD_NOT_ALLOWED);
            }
        } else {
            $this->_response(['status' => FALSE, 'error' => 'Unknown method'], $this->HTTP_METHOD_NOT_ALLOWED);
        }
    ...
    public function _limit_method(array $data)
    {
        // check limit number
        if (!isset($data[0])) {
            $this->_response(['status' => FALSE, 'error' => 'Limit Number Required'], $this->HTTP_BAD_REQUEST);
        }

        // check limit type
        if (!isset($data[1])) {
            $this->_response(['status' => FALSE, 'error' => 'Limit Type Required'], $this->HTTP_BAD_REQUEST);
        }
        
        if (!isset($this->db)) {
            $this->_response(['status' => FALSE, 'error' => 'Load CodeIgniter Database Library'], $this->HTTP_BAD_REQUEST);
        }

        // check limit database table exists
        if (!$this->db->table_exists($this->API_LIMIT_TABLE_NAME)) {
            $this->_response(['status' => FALSE, 'error' => 'Create API Limit Database Table'], $this->HTTP_BAD_REQUEST);
        }
        ...
        if ($limit_type == 'ip')
        {
            ...
            if ($this->db->affected_rows() >= $limit_num)
            {
                ...
                if (isset($limit_time) AND !empty($limit_time))
                {
                    ...
                    if (is_numeric($limit_time))
                    {
                        ...
                        if ($this->db->affected_rows() >= $limit_num)
                        {
                            $this->_response(['status' => FALSE, 'error' => 'This IP Address has reached the time limit for this method'], $this->HTTP_REQUEST_TIMEOUT);
                        } else
                        {
                            // insert limit data
                    ...
                    $this->_response(['status' => FALSE, 'error' => 'This IP Address has reached limit for this method'], $this->HTTP_REQUEST_TIMEOUT);
                }
            ...
            $this->_response(['status' => FALSE, 'error' => 'Limit Type Invalid'], $this->HTTP_BAD_REQUEST);
        }
    }
    ...
    private function _api_key(array $key)
    {
            ...
            if (isset($is_header['status']) === TRUE)
            {
                ...
                if ($api_key != "table")
                {
                    if ($HEADER_VALUE != $api_key) {
                        $this->_response(['status' => FALSE, 'error' => 'API Key Invalid'], $this->HTTP_UNAUTHORIZED);
                    }
                ...
                } else
                {
                    ...
                    if (!$this->db->table_exists($this->API_KEYS_TABLE_NAME)) {
                        $this->_response(['status' => FALSE, 'error' => 'Create API Key Database Table'], $this->HTTP_BAD_REQUEST);
                    }
                    ...
                    {
                        $this->_response(['status' => FALSE, 'error' => 'API Key Invalid'], $this->HTTP_NOT_FOUND);
                    } 
                }

            } else
            {
                $this->_response(['status' => FALSE, 'error' => 'Set API Key in Request Header'], $this->HTTP_NOT_FOUND);
            }
        } else if (strtolower($api_key_type) == 'get') // // api key type `get`
        {
            ...
            {
                $this->_response(['status' => FALSE, 'error' => 'API Key GET Parameter Required'], $this->HTTP_NOT_FOUND);
            }
        } else if (strtolower($api_key_type) == 'post') // // api key type `post`
        {
            ...
            {
                $this->_response(['status' => FALSE, 'error' => 'API Key POST Parameter Required'], $this->HTTP_NOT_FOUND);
            }

        } else {
            $this->_response(['status' => FALSE, 'error' => 'API Key Parameter Required'], $this->HTTP_NOT_FOUND);
        }
    }
